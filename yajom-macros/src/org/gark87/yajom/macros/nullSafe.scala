...
object nullSafe {
  ...
  : c.Expr[Unit] = {
    ...
    if (fromType == toType) {
      ...
      val guards = withNullGuards(c)(setter, creator, objectFactoryType)
      c.Expr[Unit](reify {
  ...
  def withNullGuards[T: c.WeakTypeTag](c: reflect.macros.Context)(expr: c.Expr[(T) => Unit], creator: ObjectCreator, objectFactoryType: c.Type): c.Expr[(T) => Unit] = {
    import c.universe._
